...
def evaluate_mip_df(eval_MDP, eval_MDP_ID, no_evaluations):
    ...
    while evaluation_no < no_evaluations:
        ...
        df.loc[evaluation_no, 'l1_norm'] = int(1 / eval_MDP.reward(sim_history[-1,:3])) - 1
        df.loc[evaluation_no, 'no_solutions'] = sim_mip_solutions
        df.loc[evaluation_no, 'solver_time'] = sim_compute_time
        evaluation_no += 1
...
if __name__ == "__main__":
    os.system('clear')
    MDP = dp3.MarkovGridWorld(grid_size=10, direction_probability=0.9,obstacles=np.array([[0,0], [4,4]]), landing_zone = np.array([1,1]), max_altitude = 20)
    no_evaluations = 20
