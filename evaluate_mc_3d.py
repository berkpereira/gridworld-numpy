...
def epsilon_train_policies(MDP, epsilon_params, evaluation_no_episodes, no_improvement_steps):
    for epsilon in epsilon_params:
        ...
        trained_policy, trained_policy_array = mc3.monte_carlo_policy_iteration(initial_policy, MDP, epsilon, evaluation_no_episodes, no_improvement_steps)
        
        file_name = 'trained_array_epsilon_' + str(epsilon)
        file_name = file_name.replace('.', ',') # get rid of dots in file name
        np.save('results/3d/training_epsilon/' + file_name, trained_policy_array)
...
def epsilon_evaluate_policies(MDP, no_evaluations, epsilon_train_params):
    ...
    for train_epsilon in epsilon_train_params:
        ...
        file_name = 'results/3d/training_epsilon/trained_array_epsilon_' + str(train_epsilon)
...
def save_epsilon_results(evaluations_array, crashes_array, no_evaluations, train_epsilon_params, epsilon_wind_param, this_dir=True):
    if this_dir is False:
        evaluations_file_name = 'results/3d/training_epsilon/epsilon_evaluations_array.txt'
        crashes_file_name = 'results/3d/training_epsilon/epsilon_crashes_array.txt'
        info_file_name = 'results/3d/training_epsilon/epsilon_evaluations_info.txt'
    else:
...
def ratio_steps_train_policies(MDP, no_episodes_ratio_params, no_steps_params):
    for no_episodes_ratio in no_episodes_ratio_params:
        ...
        for no_steps in no_steps_params:
            ...
            initial_policy = dp3.random_walk
            trained_policy, trained_policy_array = mc3.monte_carlo_policy_iteration(initial_policy, MDP, bp3.ratio_episodes_steps_epsilon, no_episodes, no_steps)
            ...
            np.save('results/3d/training_ratio_steps/' + file_name, trained_policy_array)
...
def ratio_steps_evaluate_policies(MDP, no_evaluations, no_episodes_ratio_params, no_steps_params):
    ...
    for no_episodes_ratio in no_episodes_ratio_params:
        ...
        for no_steps in no_steps_params:
            ...
            # fetch the corresponding pre-computed policy
            file_name = 'results/3d/training_ratio_steps/trained_array_ratio_' + str(no_episodes_ratio) + '_steps_' + str(no_steps)
            file_name = file_name.replace('.', ',')
            file_name = file_name + '.npy'
            policy_array = np.load(file_name)
            policy = dp3.array_to_policy(policy_array, MDP)
...
def save_ratio_steps_results(evaluations_array, crashes_array, no_evaluations, no_episodes_ratio_params, no_steps_params, state_space_size, this_dir=True):
    if this_dir is False:
        evaluations_file_name = 'results/3d/training_ratio_steps/ratio_steps_evaluations_array.txt'
        crashes_file_name = 'results/3d/training_ratio_steps/ratio_steps_crashes_array.txt'
        info_file_name = 'results/3d/training_ratio_steps/ratio_steps_evaluations_info.txt'
    else:
...
if __name__ == "__main__":
    ...
    if epsilon_evaluate:
        evaluations, crashes = epsilon_evaluate_policies(bp3.epsilon_MDP, bp3.epsilon_no_evaluations, bp3.epsilon_train_params)
        print(f'Evaluated policies using {bp3.epsilon_no_evaluations} simulations each.')
        print(f'Evaluated policies trained with following epsilon parameters: {bp3.epsilon_train_params}')
    ...
    if ratio_steps_evaluate:
        evaluations, crashes = ratio_steps_evaluate_policies(bp3.epsilon_MDP, bp3.epsilon_no_evaluations, bp3.ratio_episodes_steps_ratio_params, bp3.ratio_episodes_steps_no_steps_params)
        print(f'Evaluated policies using {bp3.epsilon_no_evaluations} simulations each.')
        print(f'Evaluated policies trained with following ratios of no. episodes to state space sizes parameters: {bp3.ratio_episodes_steps_ratio_params}')
        print(f'Evaluated policies trained with following number of improvement steps: {bp3.ratio_episodes_steps_no_steps_params}')
        print('Evaluations array:')
        print(evaluations)
        print('Crashes array:')
        print(crashes)
    ...
    if ratio_steps_plot:
        ...
        ratio_steps_plot_evaluations(evaluations_file, bp3.ratio_episodes_steps_ratio_params, bp3.ratio_episodes_steps_no_steps_params, surface = False, save = False)
        ratio_steps_plot_crash_rates(crashes_file, bp3.epsilon_no_evaluations, bp3.ratio_episodes_steps_ratio_params, bp3.ratio_episodes_steps_no_steps_params, surface = False, save = False)
        plt.show()